...
impl TryFrom<&[i16]> for Color {
    ...
    fn try_from(slice: &[i16]) -> Result<Self, Self::Error> {
        match slice {
            [r, g, b] => {
                let red =
                    u8::try_from(*r).map_err(|_| IntoColorError::IntConversion)?;
                let green =
                    u8::try_from(*g).map_err(|_| IntoColorError::IntConversion)?;
                let blue =
                    u8::try_from(*b).map_err(|_| IntoColorError::IntConversion)?;
                Ok(Color { red, green, blue })
            }
